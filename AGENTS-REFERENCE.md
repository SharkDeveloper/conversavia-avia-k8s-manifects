...
| Файл | ApplicationSet | Тип source | Генерирует |
|---|---|---|---|
| [`appset-rp.store.yaml`](appset-rp.store.yaml:1) | `rp.store` | Kustomize (путь) | `postgres`, `redis` |
| [`appset-rp.store-inventree.yaml`](appset-rp.store-inventree.yaml:1) | `rp.store-inventree` | Helm (`chart:` + `valueFiles`) | `inventree` |
...
### 3.2. Содержимое [`appset-rp.store.yaml`](appset-rp.store.yaml:1)
...
### 3.3. Содержимое [`appset-rp.store-inventree.yaml`](appset-rp.store-inventree.yaml:1)
...
2. Добавить элемент в `list.elements` в [`appset-rp.store.yaml`](appset-rp.store.yaml:1).
...
## 9. Чек-лист для нового разработчика/нейронки
